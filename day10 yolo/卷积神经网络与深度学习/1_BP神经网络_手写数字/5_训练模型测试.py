...
def train(dataloader, model, loss_fn, optimizer):
    ...
    for x, y in dataloader:  # 遍历数据加载器中的每个训练批次
        ...
        print(f'loss:{loss_value:.4f}[batch_num:{batch_size_num}]')
        batch_size_num += 1
...
def test(dataloader, model, loss_fn):
    ...
    model.eval()  # 模型设置评估模式,模型不会进行梯度计算和参数更新，专注于进行推断（预测）。
    test_loss, correct = 0, 0  # 测试损失和正确预测
    with torch.no_grad():  # 禁用梯度计算。在测试阶段，不需要计算梯度
        for x, y in dataloader:  # 遍历测试数据加载器中的每个测试批次
            x, y = x.to(device), y.to(device)
            pred = model.forward(x)  # 模型前向传播，计算输入数据预测值
            test_loss += loss_fn(pred, y).item()  # 计算当前批次的损失值
            correct += (pred.argmax(1) == y).type(torch.float).sum().item()  # 计算当前批次中正确预测的样本数量
